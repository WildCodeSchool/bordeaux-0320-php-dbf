...
document.addEventListener('DOMContentLoaded',() => {
    ...
    const editorbtns= document.getElementsByClassName('edit-subject-button');
    for (let i = 0; i < editorbtns.length; i++) {
        editorbtns[i].addEventListener('click', (event) => {
            editor(editorbtns[i], `/subject/${editorbtns[i].dataset.target}/edit`, 'editor-line', 'edit-subject-button')
        });
    }
